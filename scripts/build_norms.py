...
from open_normative.normative import build_normative, _PERCENTILE_POINTS, _assign_bin
...
def write_provenance_md(subjects_for_norms: list, output_dir: Path, age_bins: list) -> None:
    """Write PROVENANCE.md to output_dir.

    Contains:
    - Opening paragraph with timestamp and format info.
    - PSD freq-resolved category section.
    - Cohort sex breakdown table per age bin and condition.

    Args:
        subjects_for_norms: List of subject dicts (must have keys: age, sex, condition).
        output_dir: Directory where PROVENANCE.md will be written.
        age_bins: Sorted list of bin edges passed to _assign_bin.
    """
    ...
    # ------------------------------------------------------------------ #
    # Count F / M / Other per (bin, condition)
    # ------------------------------------------------------------------ #
    counts: dict[tuple[str, str], dict[str, int]] = defaultdict(
        lambda: {"F": 0, "M": 0, "Other/unknown": 0}
    )
    ...
    for s in subjects_for_norms:
        ...
        # Skip subjects with unusable age
        try:
            age_float = float(age)
        except (TypeError, ValueError):
            continue

        bin_label = _assign_bin(age_float, age_bins)
        if bin_label is None:
            continue

        sex_key = raw_sex if raw_sex in ("F", "M") else "Other/unknown"
        counts[(bin_label, cond)][sex_key] += 1

    # Build sorted rows: sort by bin edges then condition
    def _bin_sort_key(bl: str) -> int:
        """Return the lower edge of a bin label like '20-29'."""
        try:
            return int(bl.split("-")[0])
        except (ValueError, IndexError):
            return 0

    rows = sorted(counts.keys(), key=lambda k: (_bin_sort_key(k[0]), k[1]))

    table_lines = [
        "| Age bin | Condition | F | M | Other/unknown | Total |",
        "|---|---|---|---|---|---|",
    ]
    for bin_label, cond in rows:
        c = counts[(bin_label, cond)]
        f_n = c["F"]
        m_n = c["M"]
        ou_n = c["Other/unknown"]
        total = f_n + m_n + ou_n
        table_lines.append(f"| {bin_label} | {cond} | {f_n} | {m_n} | {ou_n} | {total} |")
    ...
    # ------------------------------------------------------------------ #
    content = f"""# Open Normative — Provenance

Generated by `scripts/build_norms.py` at {timestamp}.
Bundle format: NPZ v3 (sex-stratified), JSON v2 (with sex field), CSV.
...
"""
    ...
    (output_dir / "PROVENANCE.md").write_text(content, encoding="utf-8")
...
def main():
    ...
    if args.merge:
        ...
        write_provenance_md(subjects_for_norms, output_dir, args.age_bins)
        logger.info(f"  {output_dir / 'PROVENANCE.md'}")
