#!/usr/bin/env python3
"""Download Test-Retest EEG dataset from OpenNeuro (ds004148).

60 healthy adults ages 18-28, 64-channel BrainVision, CC0 license.
Eyes-closed and eyes-open resting state (~5 min each), 500 Hz, FCz reference.
Recorded in China (50 Hz line frequency).

Data layout on S3:
    s3://openneuro.org/ds004148/
        participants.tsv
        sub-01/ses-session1/eeg/sub-01_ses-session1_task-eyesclosed_eeg.{vhdr,eeg,vmrk}
        sub-01/ses-session1/eeg/sub-01_ses-session1_task-eyesopen_eeg.{vhdr,eeg,vmrk}
        ...

Only session 1 is downloaded (session 2/3 excluded for normative use).
Only task-eyesclosed and task-eyesopen are downloaded (other tasks excluded).

Usage:
    python scripts/trt_download.py ~/Data/EEG/TRT

    # Dry run
    python scripts/trt_download.py ~/Data/EEG/TRT --dry-run

    # First 10 subjects
    python scripts/trt_download.py ~/Data/EEG/TRT --max-subjects 10
...
logger = logging.getLogger("trt_download")

S3_BUCKET = "s3://openneuro.org/ds004148"
...
def download_subject(dest_dir, sid, dry_run=False, max_retries=3):
    """Download session 1 EEG data for one subject (eyesclosed + eyesopen only)."""
    local_eeg = dest_dir / sid / "ses-session1" / "eeg"

    # Check if already downloaded (need both tasks)
    if local_eeg.exists():
        ec_files = list(local_eeg.glob("*task-eyesclosed*"))
        eo_files = list(local_eeg.glob("*task-eyesopen*"))
        if ec_files and eo_files:
            return "skipped"

    s3_src = f"{S3_BUCKET}/{sid}/ses-session1/eeg/"
    ...
    for attempt in range(max_retries):
        try:
            run_aws([
                ...
                "--exclude", "*",
                "--include", "*task-eyesclosed*",
                "--include", "*task-eyesopen*",
            ], dry_run=dry_run)
            if dry_run or (
                any(local_eeg.glob("*task-eyesclosed*.vhdr"))
                and any(local_eeg.glob("*task-eyesopen*.vhdr"))
            ):
                return "downloaded"
...
def main():
    parser = argparse.ArgumentParser(
        description="Download Test-Retest EEG data from OpenNeuro",
    ...
    # ~37 MB per subject for 2 tasks (eyesclosed + eyesopen)
    est_gb = len(subjects) * 37 / 1024
    logger.info(
        "\nDownload plan:\n"
        "  Dataset: Test-Retest EEG (ds004148)\n"
        "  Subjects: %d\n"
        "  Tasks: eyesclosed, eyesopen (session 1 only)\n"
