"""MIPDB (Multimodal Resource for Studying Information Processing in the Developing Brain) dataset loader.

The MIPDB dataset contains resting-state EEG recordings using 128-channel
EGI HydroCel Geodesic Sensor Nets from subjects aged 6–44.

Reference: Langer et al. (2017). A resource for assessing information
processing in the developing brain using EEG and eye tracking. Scientific
Data, 4, 170040.
https://doi.org/10.1038/sdata.2017.40

Download from: http://fcon_1000.projects.nitrc.org/indi/cmi_eeg/
"""
...
MIPDB dataset must be downloaded from INDI.

1. Visit: http://fcon_1000.projects.nitrc.org/indi/cmi_eeg/
2. Download the EEG data and organize into BIDS-like layout:
   mipdb/
...
See: https://doi.org/10.1038/sdata.2017.40 for full documentation.
"""
...
class MIPDBLoader(DatasetLoader):
    ...
    Expects a BIDS-like directory layout with 128-channel EGI files
    (.raw or .mff) and a participants.tsv with subject demographics.
    """
    ...
    def iter_subjects(self, data_dir: Path) -> Iterator[SubjectRecord]:
        """Yield SubjectRecord for each EO/EC file in the MIPDB dataset.
        ...
        # Look for .raw files and .mff directories
        eeg_paths = sorted(
        ...
        for eeg_path in eeg_paths:
            subject_id = eeg_path.parts[-3]  # sub-XXXX
